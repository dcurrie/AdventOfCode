    ...
    while line do
        if line:len() ~= xdim then 
            ...
            for i = 1, xdim do
                ...
                elseif c == 'E' then
                    row[i] = c
                    elves[#elves+1] = {i = #elves+1, c=c, t='G', ts=grems, x = i, y = ydim, pwr=pwr, hit=200}
                else
...
function round (pwr)
    ...
    for i = 1, 100000000 do
        step(i)
        if #elves == 0 then
            if verbose then print("Part 1 ends after", i, "-", partialstep) end
            score = 0
            for j = 1, #grems do
                score = score + ((i - partialstep) * grems[j].hit)
            end
            if pwr == 3 then print("Part 1: Gremlins win, score is", score) end
            if verbose then showarena (i) end
            score = -score
            break
        elseif #grems == 0 then
            if verbose then print("Part 1 ends after", i, "-", partialstep) end
            ...
            if pwr == 3 then print("Part 1: Elves win, score is", score) end
            if verbose then showarena (i) end
            break
        else
            if verbose then
...
function part2 ()
    local guesslo = 3 -- always a loser
    local guesshi = 8
    local score
    local hiscore
    ...
    repeat
        guesshi = guesshi * 2
        score = round(guesshi)
        print("Try up", guesshi, score, #deadelves)
        if score > 0 and #deadelves == 0 then
            -- ok
        else
            guesslo = guesshi
        end
    until score > 0 and #deadelves == 0 
    ...
    repeat 
        ...
        score = round(guess)
        print("Try dn", guess, score, #deadelves)
        if score > 0 and #deadelves == 0 then
            guesshi = guess
            hiscore = score
        else
