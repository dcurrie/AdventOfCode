...
local function part2 (n)
    ...
    for i = 1, n do
        ...
        if i > 100 then
            local trees, yards = score()
            --printf("After %4d steps %4d trees %4d yards\n", i, trees, yards)
            sh = trees * 5000 + yards
            if scorehash[sh] then
                printf("After %4d and %d (delta %d) steps %4d trees %4d yards\n",
                         scorehash[sh], i, i - scorehash[sh], trees, yards)
            else
                scorehash[sh] = i
            end
        end
...
--[[
    ...
    printf("Part2 resource value = %d x %d = %d\n", trees, yards, trees * yards)

-[[ from dribble, pattern starts at 509 and 537 (delta 28) steps  601 trees  352 yards

    (1000000000 - 509) % 28 = 15
    at step 509 + 15 = 524 the score is 596 trees  331 yards = 197276
]]
