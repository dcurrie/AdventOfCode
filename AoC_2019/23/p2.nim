...
var nicchans: array[0..50, Channel[(int,int)]]
...
proc nic(npgm: tuple[n: int, src: seq[string]]) {.thread.} =
    ...
    while not m.halted:
        ...
        if t <= 49:
            ...
            let x = run2o(m)
            let y = run2o(m)
            send(nicchans[50], (x, y))
        else:
            echo "Wtf @", n, " got: ", t
    if verbo: echo "Terminating # ", n

proc nat(npgm: tuple[n: int, src: seq[string]]) {.thread.} =
    let (n, _) = npgm
    assert n == 50
    var (x, y) = (-1, -1)
    var ylast = int.high
    while true:
        let (ok, v) = tryRecv(nicchans[50])
        if ok:
            if verbo: echo "Receiving @", 50, " ", v
            (x, y) = v
        var idlers = 0
        for i in 0..50:
            if nicchans[i].peek <= 0: inc idlers
            else: break
        if idlers == 51:
            sleep(1)
            for i in 0..50:
...
proc part2() = 
    ...
    for i in 0..50:
        ...
        open(nicchans[i], 0)
    var thr: array[0..50, Thread[tuple[n: int, src: seq[string]]]]
    for i in 0..49:
        createThread(thr[i], nic, (i, input))
    createThread(thr[50], nat, (50, input))
    joinThreads(thr)
